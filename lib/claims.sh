...
update_claim() {
  ...
  acquire_lock "$CLAIMS_LOCK" || { log_err "Could not acquire lock for claims: $CLAIMS_LOCK"; return 1; }
  tmp_file="$(mktemp "${CLAIMS_FILE}.XXXXXX")" || { log_err "Failed to create temp claims file: $CLAIMS_FILE"; release_lock "$CLAIMS_LOCK"; return 1; }
